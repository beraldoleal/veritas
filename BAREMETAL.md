...
> [!WARNING]
> The initrd measurement event in rtmr_2 may be dropped upstream.
> If that happens, rtmr_2 would only cover the kernel cmdline and
> its hash would change. Veritas (via tdx-measure) would need to
> stop including the initrd when computing rtmr_2. CI validation
> against the attestation policy would catch this.
